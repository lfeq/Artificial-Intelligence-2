...
public class Graph {
    ...
    private List<Node> porAnalizar = new List<Node>();
    private List<Node> shortestPath = new List<Node>();
    private bool aStarFound = false;
    ...
    private void Analizar(Node t_node) {
        ...
        for (int i = 0; i < t_node.GetEdges().Count; i++) {
            ...
            {
                if(tempEdge.getNodeTo().GetCaminoRecorrido() > (tempEdge.getDistance() + t_node.GetCaminoRecorrido())){
                    tempEdge.getNodeTo().SetCaminoRecorrido(tempEdge.getDistance() + t_node.GetCaminoRecorrido());
                    tempEdge.getNodeTo().SetCost(cost);
                    tempEdge.getNodeTo().SetCorrectEdge(tempEdge);
                }        
        ...
        porAnalizar.Add(t_node.GetEdges()[smallestNodeCostIndex].getNodeTo());
        //for (int i = 0; i < t_node.GetEdges().Count; i++) {
        ...
        for(int i = 0; i < t_node.GetEdges().Count; i++)
        {
            porAnalizar.Add(t_node.GetEdges()[i].getNodeTo());
        }
        SortList();
    }
    private void SortList()
    {
        porAnalizar.Sort(delegate (Node c1, Node c2)
        {
            return c1.GetCost().CompareTo(c2.GetCost());
        });
    }

    private void SetShortestPath(Node initialNode)
    {
        Node tempNode = finalNode;
        while(tempNode != initialNode)
        {
            shortestPath.Add(tempNode);
            tempNode = tempNode.GetCorrectEdge().getNodeFrom();
        }
    }
}