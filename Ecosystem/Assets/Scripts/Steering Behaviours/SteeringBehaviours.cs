...
public class SteeringBehaviours {
    ...
    public static Vector3 seek(BaseAgent t_BaseAgent, Vector3 t_target, bool useArrival = false) {
        ...
        desiredVelocity *= t_BaseAgent.getMaxSpeed();
        if (useArrival) {
    ...
    public static Vector3 pursuit(BaseAgent t_BaseAgent, BaseAgent t_BaseAgentToPursuit) {
        ...
        float positionPrediction = distanceToTarget / t_BaseAgent.getMaxSpeed();
    ...
    public static Vector3 wander(BaseAgent t_BaseAgent) {
        ...
        circleCenter *= t_BaseAgent.getCircleDistance();
        Vector3 displacement = new Vector3(0, 0, -1);
        displacement *= t_BaseAgent.getCircleRadius();
        float displacementMagnitud = displacement.magnitude;
        float angleChange = t_BaseAgent.getAngleChange();
        ...
        wanderForce = Vector3.ClampMagnitude(wanderForce, t_BaseAgent.getMaxSpeed());
        wanderForce /= t_BaseAgent.getMass();
        return Vector3.ClampMagnitude((t_BaseAgent.getCurrentVelocity() + wanderForce), t_BaseAgent.getMaxSpeed());
    }

    public static Vector3 collisionAvoidance(BaseAgent t_BaseAgent) {
        Vector3 agentPosition = t_BaseAgent.transform.position;
        float dynamicLenght = t_BaseAgent.getCurrentVelocity().magnitude / t_BaseAgent.getMaxSpeed();
        ...
        GameObject mostThreateningObstacle = findMostThreateningObstacle(ahead, ahead2, t_BaseAgent.getCollisionObstacleAvoidanceRadius(), agentPosition);
        ...
        avoidance.Normalize();
        avoidance *= t_BaseAgent.getCollisionAvoidanceForce();
    ...
    private static Vector3 arrival(BaseAgent t_BaseAgent, Vector3 target, Vector3 desiredVelocity) {
        ...
        if (distance <= t_BaseAgent.getSlowingRadius()) {
            desiredVelocity.Normalize();
            desiredVelocity *= t_BaseAgent.getMaxSpeed() * (distance / t_BaseAgent.getSlowingRadius());
        }
    ...
    private static Vector3 calculateSteeringForce(BaseAgent t_BaseAgent, Vector3 t_desiredVelocity) {
        ...
        steeringForce = Vector3.ClampMagnitude(steeringForce, t_BaseAgent.getMaxForce());
        steeringForce /= t_BaseAgent.getMass(); ;
        return Vector3.ClampMagnitude((t_BaseAgent.getCurrentVelocity() + steeringForce), t_BaseAgent.getMaxSpeed());
    }
